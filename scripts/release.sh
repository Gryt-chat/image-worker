...
if [ "$RERELEASE" = false ]; then
  ...
  if [ -n "$REPO_ROOT" ] && [ -f "$REPO_ROOT/.gitmodules" ]; then
    ...
    git commit -m "release: image-worker v${NEW_VERSION} (${CHANNEL})"
...
if [[ "$DEPLOY_BETA" =~ ^[Yy]$ ]]; then
  ...
  if [ -n "$REPO_ROOT" ] && [ -f "$BETA_COMPOSE" ] && [ -f "$BETA_ENV" ]; then
    ...
    docker compose "${COMPOSE_ARGS[@]}" up -d image-worker
    if docker compose "${COMPOSE_ARGS[@]}" ps --format '{{.Name}}' 2>/dev/null | grep -q image-worker-nt; then
      ...
      docker compose "${COMPOSE_ARGS[@]}" up -d image-worker-nt
    fi
...
if [ "$CHANNEL" = "latest" ]; then
  read -rp "$(echo -e "${CYAN}?${RESET}  Deploy image-worker to production? ${YELLOW}[Y/n]${RESET}: ")" DEPLOY_PROD
  ...
  if [[ "$DEPLOY_PROD" =~ ^[Yy]$ ]]; then
    ...
    if [ -n "$REPO_ROOT" ] && [ -f "$PROD_COMPOSE" ] && [ -f "$PROD_ENV" ]; then
      ...
      info "Pulling & restarting production image-worker…"
      docker compose "${COMPOSE_ARGS[@]}" pull image-worker
      docker compose "${COMPOSE_ARGS[@]}" up -d image-worker
      if docker compose "${COMPOSE_ARGS[@]}" ps --format '{{.Name}}' 2>/dev/null | grep -q image-worker-nt; then
        docker compose "${COMPOSE_ARGS[@]}" pull image-worker-nt
      ...
      ok "Production image-worker deployed"
    else
